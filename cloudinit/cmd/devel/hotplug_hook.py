...
class NetHandler(UeventHandler):
    def __init__(self, ds, devpath, success_fn):
        ...
        log_console('self.devpath is: {}'.format(self.devpath))
    ...
    def detect(self, action):
        ...
        log_console('detect_presence is: {}'.format(detect_presence))
        log_console('self.id is: {}'.format(self.id))
        log_console('self.config is: {}'.format(self.config))
...
SUBSYSTEM_TO_EVENT = {
    'net': EventScope.NETWORK,
    'block': EventScope.STORAGE,
}
...
def handle_args(name, args):
    ...
    with hotplug_reporter:
        ...
        # In the future we may want disk events, but for now just check
        # if the subevent is 'net'
        ...
        for attempt, wait in enumerate(retries):
            ...
            try:
                ...
                log_console('udevaction is: {}'.format(args.udevaction))
